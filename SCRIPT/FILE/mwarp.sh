...
select_proxy() {
    ...
    count=$(proxy_count)
    if (( count == 0 )); then
        echo "No proxy found." >&2
        return 1
    fi

    # Paparan dihantar ke stderr supaya command substitution hanya
    # menangkap nama proxy yang dipilih, bukan keseluruhan menu.
    i=1
    while IFS= read -r name; do
        host=$(proxy_value "$name" host)
        port=$(proxy_value "$name" port)
        type=$(proxy_value "$name" type)
        status=$(proxy_value "$name" status)
        printf '%d) %-18s %-7s %-28s [%s]\n' \
            "$i" "$name" "$type" "$host:$port" "${status^^}" >&2
        ((i++))
    done < <(proxy_names)

    while true; do
        read -rp "Select proxy [1-$count] or 0 to cancel: " choice >&2
        [[ "$choice" == "0" ]] && return 1
